  ...
  while (!hasError) {
    ...
    try {
      ...
      if (error.message.includes("Status code: 500")) {
        console.log(`Received a 500 error for ${sport} ${year}. Moving to the next sport.`);
        return; // Exit the function and move to the next sport
      }
      console.log(`Reached end of pages for ${sport} ${year} at page ${formattedPageNumber}`);
      hasError = true;
    }
// @ts-nocheck
import https from "https";
...
async function crawlAllPages(sport, baseUrl, year) {
  const outputDir = `src/app/assets/books/${sport}/${year}/html`;
  let pageNumber = 1;
  let hasError = false;
