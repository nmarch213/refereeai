...
### #rule-11-4
**ART. 4.** Only one 60-second time-out is charged (or one 30-second time-out, if that is the only type of time-out remaining) in [Rule 5-8-4](#rule-5-8-4) regardless of the amount of time consumed when no correction is made. 

> **EXCEPTION:** No time-out is charged:
> - a. If, in [Rule 5-8-3](#rule-5-8-3), the player's request results from displaced eyeglasses or lens.
> - b. If, in [Rule 5-8-4](#rule-5-8-4), the error or mistake is prevented or rectified.
```
